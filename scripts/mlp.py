...
def get_data(
    ...
    valid_y_1 = [train_y_1[s] for s in sidx[n_train:]]
    valid_y_2 = [train_y_2[s] for s in sidx[n_train:]]
    valid_y_3 = [train_y_3[s] for s in sidx[n_train:]]
    train_desc = [train_desc[s] for s in sidx[:n_train]]
    train_brands = [train_brands[s] for s in sidx[:n_train]]
    train_y_1 = [train_y_1[s] for s in sidx[:n_train]]
    train_y_2 = [train_y_2[s] for s in sidx[:n_train]]
    train_y_3 = [train_y_3[s] for s in sidx[:n_train]]
...
def train_model(model='custom_mlp: ',
    ...
    if cat != 1:
        prev_cat_var = T.matrix('prev_inputs',dtype='int32')
...
def train_simple_model(model='custom_mlp', 
    ...
    for n in network:
        ...
        updates.append(lasagne.updates.adadelta(
            loss, p, learning_rate=0.01))
        params.append(p)
    ...
    for epoch in range(num_epochs):
        # In each epoch, we do a full pass over the training data:
        train_err = np.zeros(num_targets)
        train_batches = 0
        start_time = time.time()
        t_idx = 0
        for batch in iterate_minibatches(train[0], train[1], batch_size, shuffle=True):
...
def main():
    ...
    data, n_values = get_data(
        test_size=7500,  # If >0, we keep only this number of test example.
