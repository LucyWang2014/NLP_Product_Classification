...
#intially, all params are initialized on the cpu, and then pushed to gpu with tparams
...
def build_model(tparams, options):
    ...
    emb = tparams['Wemb'][x.flatten()].reshape([n_timesteps,
                                                n_samples,
                                                options['dim_proj']])
    #if there's another set of lstm features, would there be another tparams matched to the prepared
    #data of product names, and a second proj
    #then another vector of size count of brands * ydim, dot the brand names
    #where does the optimization for the proj and brand names happen?
    ...
    #concatenate features to projections, change dimensions of U
    #projection of the layers into the features
...
def pred_probs(f_pred_prob, prepare_data, data, iterator, categories, verbose=False):
    ...
    for _, valid_index in iterator:
        x, mask, y = prepare_data([data[0][t] for t in valid_index],
                                  ...
                                  maxlen=None)
        #with extra feature sets, here it would be f_pred_prob(x,x_brand,x_name,mask)?
...
def pred(f_pred, prepare_data, data, iterator, verbose=False):
    ...
    for _, valid_index in iterator:
        x, mask, y = prepare_data([data[0][t] for t in valid_index],
                                  numpy.array(data[1])[valid_index],
                                  maxlen=None)
        ##with extra feature sets, here it would be f_pred(x,x_brand,x_name,mask)?
...
def final_errors(predictions, target, verbose=False):
    ...
    #data = np.dstack(predictions, target)
...
def train_lstm(
    ...
    categories = len(numpy.unique(train[1]))
    ...
    #would there be 3 grads for each corresponding set of params?
    ...
    prediction_probs = pred_probs(f_pred_prob, prepare_data,test, categories, kf_test)
...
def main():
    ...
    train_err_3,valid_err_3, test_err_3, history_errs, predictions_3, prediction_probs_3 = train_lstm(
        data = data_3,
        max_epochs=max_epochs,
        cat_level = 3,
        dataset='nordstrom',
